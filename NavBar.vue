<template>
  <header class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-50">
    <div class="max-w-7xl mx-auto px-4 py-3 md:py-4 flex items-center justify-between gap-3 relative">
      <router-link to="/" class="flex items-center shrink-0">
        <img src="/shopsphere-logo.svg" alt="ShopSphere Logo" class="h-9 md:h-12 w-auto max-w-[9rem] md:max-w-none">
      </router-link>
      ...
      <nav class="hidden md:flex gap-6 items-center">
        ...
        <div class="relative" ref="cartRef">
          ...
          <div 
            ...
            :class="showCart ? 'max-w-[calc(100vw-1rem)] w-[calc(100vw-1rem)] md:w-96' : ''"
          >
      ...
      <div class="flex md:hidden items-center gap-2">
        ...
        <div class="relative" ref="cartRef">
          <button
            @click="showCart = !showCart"
            class="relative cursor-pointer flex items-center p-1"
            title="Shopping Cart"
          >
            <ShoppingCartIcon class="h-7 w-7 text-gray-700 dark:text-gray-200 hover:text-indigo-600 transition-colors" />
            <span v-if="cartTotalCount > 0" class="absolute -top-1 -right-1 bg-indigo-600 text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center animate-popIn">
              {{ cartTotalCount }}
            </span>
          </button>
          ...
          <div
            v-if="showCart"
            class="absolute right-0 top-12 w-[calc(100vw-1rem)] max-w-[22rem] bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-gray-100 dark:border-gray-700 z-50 overflow-hidden"
          >
            <div class="p-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold text-lg flex justify-between items-center">
              <span>🛒 Your Cart</span>
              <button @click="showCart = false" class="text-white/80 hover:text-white text-xl">&times;</button>
    ...
    <div v-if="mobileMenuOpen" class="md:hidden border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-3 shadow-lg">
      <div class="flex flex-col gap-3">
        <router-link to="/" class="py-2 font-medium hover:text-indigo-600 transition-colors" @click="mobileMenuOpen = false">Home</router-link>
        <router-link to="/login" class="py-2 font-medium hover:text-indigo-600 transition-colors" @click="mobileMenuOpen = false">Login</router-link>
        <router-link to="/signup" class="py-2 font-medium hover:text-indigo-600 transition-colors" @click="mobileMenuOpen = false">Signup</router-link>
      </div>
    </div>
  </header>
</template>
...
import { ShoppingCartIcon, TrashIcon, SunIcon, MoonIcon, Bars3Icon, XMarkIcon } from '@heroicons/vue/24/outline'
